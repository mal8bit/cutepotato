...
namespace Cutexe
{
    internal class ArgsParse
    {
        ...
        public static T ParseArgs<T>(string[] args)
        {
            ...
            {
                throw new Exception($"Requ ired Parameter {string.Join(",", requiredPropertyList.ToArray())}");
            }
        ...
        public static string PrintHelp(Type type,string head,string appName, string[] examples) {;
            ...
            {
                stringBuilder.WriteLine("\t-{0} Requ ired:{1} {2} (default {3})", argsAttribute.FieldName, argsAttribute.Required, argsAttribute.Description, argsAttribute.DefaultValue); 
            }
            stringBuilder.WriteLine();
