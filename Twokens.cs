...
namespace Twokens
{
  ...
  [In] uint dwOptions
    ...
    public class TokenuUils
    {
        ...
        {
            IntPtr cpRead = IntPtr.Zero;
            IntPtr cpWrite = IntPtr.Zero;
            ...
            if (!NativeMethod.CreatePipe(out cpRead, out cpWrite,
            ...
            startupInfo.hStdError = cpWrite;
            startupInfo.hStdOutput = cpWrite;
            startupInfo.hStdInput = IntPtr.Zero;
            startupInfo.dwFlags = (int)NativeMethod.STARTF_USESTDHANDLES;

            NativeMethod.SetHandleInformation(cpRead, NativeMethod.HANDLE_FLAG_INHERIT, NativeMethod.HANDLE_FLAG_INHERIT);
            NativeMethod.SetHandleInformation(cpWrite, NativeMethod.HANDLE_FLAG_INHERIT, NativeMethod.HANDLE_FLAG_INHERIT);
                    ...
                    out processInformation))
            {
                ...
                childProcessReadStream = new FileStream(cpRead, FileAccess.Read, false);
                ...
                while (true)
                {
                    if (!NativeMethod.PeekNamedPipe(cpRead, readBytes, (uint)readBytes.Length,
            ...
            {
                consoleWriter.WriteLine($"[!] can;t ask manager :{Marshal.GetLastWin32Error()}");
            }
        end:
            ...
            if (cpRead != IntPtr.Zero)
            {
                NativeMethod.CloseHandle(cpRead);
            }
            if (cpWrite != IntPtr.Zero)
            {
                NativeMethod.CloseHandle(cpWrite);
            }
        }
